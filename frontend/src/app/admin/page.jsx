      ...
      if (data.success) {
        ...
        if (type === 'approve') {
          ...
          if (currentBiz) {
            ...
            const referralLink = referralCode ? `https://udumalpet.co.in/register?ref=${referralCode}` : 'https://udumalpet.co.in/register';
...
https://udumalpet.co.in/${businessSlug}
        ...
        <header className="h-[76px] bg-white border-b border-slate-200/80 px-6 md:px-8 hidden md:flex items-center justify-between z-10 sticky top-0 shrink-0">
          ...
          <div className="flex items-center gap-4">
            <div className="relative" ref={notificationsRef}>
              ...
              {notificationsOpen && (
                <div className="fixed inset-x-4 top-16 sm:absolute sm:right-0 sm:left-auto sm:w-80 sm:top-auto sm:mt-2 bg-white border border-slate-200 rounded-2xl shadow-xl py-3 px-4 flex flex-col gap-2.5 animate-fadeIn z-50 text-[#001c41]">
                      ...
                      <div className="flex flex-col gap-1 text-left">
                        ...
                        <div className="flex flex-col gap-1 mt-2">
                          ...
                          {businesses.filter(b => b.parentBusinessId && b.status === 'Approved').length > 0 && (
                            <span className="text-[9.5px] text-emerald-655 font-bold w-fit bg-emerald-100/80 border border-emerald-200/50 px-1.5 py-0.5 rounded-md mt-1">
                              +{businesses.filter(b => b.parentBusinessId && b.status === 'Approved').length} branches
                            </span>
                          )}
                        </div>
                      </div>
                      <div className="h-10 w-10 bg-emerald-50 rounded-xl flex items-center justify-center text-emerald-600 border border-emerald-100/50 shrink-0">
                  ...
                  <div className="bg-white border border-slate-200/80 shadow-sm rounded-3xl p-4 sm:p-6 md:p-8 flex flex-col gap-6 text-left w-full max-w-full min-w-0 overflow-hidden">
                    <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 border-b border-slate-100 pb-5 w-full min-w-0 max-w-full">
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 self-start md:self-center w-full md:w-auto min-w-0 max-w-full">
                        ...
                        >
                          <span>Businesses Audit</span>
                          <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${
                            auditSubTab === 'Businesses' ? 'bg-white text-[#027244]' : 'bg-slate-200 text-slate-600'
                          }`}>
                                ...
                                return (
                                  <tr key={pay._id} className="hover:bg-slate-50/50">
                                    <td className="p-4 text-slate-500 font-bold font-sans">
                                      {pay.createdAt || pay.paymentDate ? new Date(pay.createdAt || pay.paymentDate).toLocaleString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: true }) : 'N/A'}
                                    </td>
                                    <td className="p-4 flex flex-col text-left">
                                              ...
                                              >
                                                <div className="flex items-center gap-2.5 min-w-0 w-full sm:flex-1">
                                                  ...
                                                  <div className="flex flex-col text-left min-w-0 flex-1">
                                                    ...
                                                    <span className="text-[9.5px] text-emerald-600 font-semibold mt-1.5 leading-normal">{count} active businesses</span>
                                                  </div>
                                                </div>
                                      ...
                                      return (
                                        ...
                                        >
                                          <div className="flex items-center gap-3 min-w-0 flex-1">
                                            ...
                                            <div className="flex flex-col text-left min-w-0 flex-1">
                                              <span className="font-bold text-xs truncate text-slate-800">{cat.categoryName}</span>
                                              <span className="text-[9.5px] text-slate-450 font-medium mt-1 leading-normal truncate">Main: {cat.parentCategory || 'Others'}</span>
                                              ...
                                              <span className="text-[9.5px] text-emerald-655 font-semibold mt-2 leading-normal">{count} active businesses</span>
                                            </div>
                                          </div>
      ...
      {confirmModal.isOpen && (
        <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs animate-fadeIn text-[#001c41]">
          <div className="w-full max-w-md bg-white border border-slate-200 shadow-2xl rounded-[24px] p-6 flex flex-col gap-5 text-left font-sans">
            ...
            <p className="text-xs font-semibold leading-relaxed text-slate-660">
              {confirmModal.message}
            </p>
            <div className="flex justify-end gap-3 pt-2">
              <button 
                onClick={() => setConfirmModal(prev => ({ ...prev, isOpen: false }))}
                className="px-4 py-2 border border-slate-202 bg-white hover:bg-slate-50 text-slate-600 text-xs font-extrabold rounded-xl transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button 
